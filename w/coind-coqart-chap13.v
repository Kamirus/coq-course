(* Coq'Art. P. Casteran after E. Gimenez *)
...
(*
CoFixpoint length {A} (l:LList A) : nat :=
  match l with
  | LNil => 0
  | LCons a l' => S (length l')
  end.
*)

CoInductive lnat := l0 | lS : lnat -> lnat.

CoFixpoint llength {A} (l:LList A) : lnat :=
  match l with
  | LNil => l0
  | LCons a l' => lS (llength l')
  end.
...
Hint Constructors  Finite: llists.
...
CoInductive bisimilar (A:Type) : LList A -> LList A -> Prop :=
 ...
 Hint Constructors  bisimilar: llists.
